...
class TestHTTPProtocols(Framework):
    ...
    def setUp(self):
        super().setUp()
        self.url = self.base_url + "fell/" + self.id
...
class FellTest(Framework):
    ...
    def test_404_if_id_is_not_int(self):
        """tests robustness of id only accepting integers above 0"""
        ids = [
            -1,
            'test',
            1.0
    ...
    def test_columns_returned(self):
        ...
        r = requests.get(url)
        retrieved_columns = [col for col in r.json()]
